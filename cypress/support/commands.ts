...
Cypress.Commands.add('fillFormWithRandomData', () => {
  ...
  cy.userGender().click();
  cy.userGenderFemale('select[aria-hidden="true"]');
    ...
    cy.valuesJson() 
      ...
      .then((text) => {
        ...
        try {
          ...
          const profileVisibilityMatch = text.match(/"profile_visibility":\s*(true|false)/);
          ...
          if (newsletterMatch && profileVisibilityMatch && usernameMatch && ageMatch && emailMatch && bioMatch && streetMatch && cityMatch && zipcodeMatch) {
            expect(newsletterMatch[1] === 'true').to.equal(formData.newsletter);
            expect(profileVisibilityMatch[1] === 'true').to.equal(formData.profile_visibility);
            ...
            throw new Error('No se encontraron todas las coincidencias en el texto');
          }
        } catch (error) {
...
Cypress.Commands.add('fillFormWithRandomDataB', () => {
    ...
    if (Object.values(accountData).every(value => value)) {
      ...
      cy.storageLimit('input[name="storage_limit"]').type(accountData.storageLimit);
      cy.contactEmail('input[name="contact_email"]').type(accountData.contactEmail);
    ...
    cy.accountType().click();
    cy.accountTypePremium('select[aria-hidden="true"]');
    ...
    cy.valuesJson() 
      ...
      .then((text) => {
        ...
        try {
          ...
          const backup_enabledMatch = text.match(/"backup_enabled":\s*(true|false)/);
          const email_notificationsMatch = text.match(/"email_notifications":\s*(true|false)/);
          const sms_notificationsMatch = text.match(/"sms_notifications":\s*(true|false)/);
          const push_notificationsMatch = text.match(/"push_notifications":\s*(true|false)/);
          const subscription_statusMatch = text.match(/"subscription_status":\s*(true|false)/);
          ...
          if (is_activeMatch && backup_enabledMatch && email_notificationsMatch && sms_notificationsMatch && push_notificationsMatch && sms_notificationsMatch && push_notificationsMatch && subscription_statusMatch && accountNameMatch && storageLimitMatch && contactEmailMatch) {
            expect(is_activeMatch[1] === 'true').to.equal(accountData.is_active);
            expect(backup_enabledMatch[1] === 'true').to.equal(accountData.backup_enabled);
            expect(email_notificationsMatch[1] === 'true').to.equal(accountData.email_notifications);
            expect(sms_notificationsMatch[1] === 'true').to.equal(accountData.sms_notifications);
            expect(push_notificationsMatch[1] === 'true').to.equal(accountData.push_notifications);
            expect(subscription_statusMatch[1] === 'true').to.equal(accountData.subscrption_status);
...
Cypress.Commands.add('fillFormWithRandomDataC', () => {
    const appData = {
      ...
      cy.environment().click();
      cy.environmentStaging('select[aria-hidden="true"]');
      cy.autoUpdate().click();
      cy.submitApp().click()
})
    ...
    cy.valuesJson() 
      ...
      .then((text) => {
        ...
        try {
          ...
          if (appNameMatch && appVersionMatch && maxUsersMatch && enableLogsMatch && featureAMatch && featureBMatch && featureCMatch && configAMatch && configBMatch && configCMatch && configDMatch && configCCMatch && configCDMatch) {
