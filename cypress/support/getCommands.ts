...
Cypress.Commands.add('userGender', () => {
...
Cypress.Commands.add('accountType', () => {
    return cy.get("[role=combobox]")
})
...
Cypress.Commands.add('environment', () => {
    return cy.get("[role=combobox]")
})

Cypress.Commands.add('environmentStaging', (comboboxSelector) => {
    cy.get('body').invoke('attr', 'style', 'pointer-events: auto')
    .then(() => {
      cy.get(comboboxSelector).select(1, { force: true })
    });
...
Cypress.Commands.add('activateAllFeatures', () => {
    cy.featuresA().click()
    cy.featuresB().click();
    cy.featuresC().click();
})
